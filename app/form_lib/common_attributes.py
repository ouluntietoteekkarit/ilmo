...
from app.form_lib.lib import BaseAttribute, StringAttribute, ATTRIBUTE_NAME_FIRSTNAME, ATTRIBUTE_NAME_LASTNAME, \
    ...
    ATTRIBUTE_NAME_ALLERGIES, ATTRIBUTE_NAME_TELEGRAM, ATTRIBUTE_NAME_IRC_NAME, ATTRIBUTE_NAME_PREFERRED_TIME
...
def make_attribute_firstnames(**extra_args: Dict[str, Any]) -> BaseAttribute:
    return StringAttribute(ATTRIBUTE_NAME_FIRSTNAME, 'Etunime(t) | First name(s)', 'Etunimi', 50, **extra_args)
...
def make_attribute_preferred_time(**extra_args: Dict[str, Any]) -> BaseAttribute:
    extra_args.setdefault("validators")
    extra_args["validators"] += [Length(max=32)]

    return StringAttribute(ATTRIBUTE_NAME_PREFERRED_TIME, 'Toivomus Kellonajasta | Preferred Time', 'Time', 50, **extra_args)
