...
_event_name = "Fragment LAN -24"
_is_enabled = False
_start_date = datetime(2024, 10, 25, 13, 37, 00)
_end_date   = datetime(2024, 10, 31, 23, 59, 00)
...
class _Controller(FormController):
    ...
    def _get_email_msg(self, recipient: BaseParticipant, model: RegistrationModel, reserve: bool) -> str:
        ...
        if reserve:
            ...
            result = ' '.join([
                make_greet_line(recipient),
                "\nOlet ilmoittautunut Fragment LAN -tapahtumaan. Tässä vielä syöttämäsi tiedot: ",
                "\n\nNimi: ", firstname, lastname,
                "\nSähköposti: ", email,
                "\n\nJos tulee kysyttävää, voit olla sähköpostitse yhteydessä tapahtumavastaavat@otit.fi."
                "\n\nÄlä vastaa tähän sähköpostiin, vastaus ei mene silloin mihinkään."
            ])
