    ...
    def _get_email_msg(self, recipient: BaseParticipant, model: RegistrationModel, reserve: bool) -> str:
        if reserve:
            return ' '.join([
                ...
                "\n\nJos tulee kysyttävää, voit olla sähköpostitse yhteydessä metukka@otit.fi."
                "\n\nÄlä vastaa tähän sähköpostiin, vastaus ei mene silloin mihinkään."
            ])
        else:
            return ' '.join([
                make_greet_line(recipient),
                """\nOlet ilmoittautunut humanöörisitseille. Sitsit järjestetään Walhallassa 23.2.2023 klo 18:00 alkaen.

Tässä vielä maksuohjeet:
Maksettava summa on 20€. 40€ jos osallistut avecin kanssa. Maksu tapahtuu tilisiirrolla
Oulun Tietoteekkarit ry:n tilille FI03 4744 3020 0116 87. Kirjoita viestikenttään oma nimesi, avecisi 
nimi ja \"humanöörisitsit\". Maksun eräpäivä on 23.2.2023.
...
def _get_quotas() -> List[Quota]:
    return [
        Quota(GUILD_OTIT, 36, 0),
        Quota(GUILD_PROSE, 36, 0),
        Quota(GUILD_COMMUNICA, 36, 0),
    ]
...
_event = Event('Humanöörisitsit', datetime(2023, 2, 1, 12, 00, 00),
               datetime(2023, 2, 15, 23, 59, 59), _get_quotas(), _types.asks_name_consent())
